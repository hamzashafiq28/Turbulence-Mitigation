...
parser.add_argument('-checkpoint', help='Pretrained model location', default='checkpoints/', type=str)
...
try:
    net.load_state_dict(torch.load(os.path.join(checkpoint,'model_best.zip')))
    net_var.load_state_dict(torch.load(os.path.join(checkpoint,'model_best_var.zip')))
...
print(f"Time taken to process 199 frames: {total_time} seconds")
